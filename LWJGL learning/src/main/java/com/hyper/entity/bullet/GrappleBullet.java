	...
	public void update() {
		...
		for(Entity e : this.world.getEntities()) if(e instanceof LivingEntity && e != this.getShooter())
			if(CollisionHandler.collides(this.hitbox, e.getHitbox())) {
				if(((LivingEntity)e).damage(new DamageSource(DamageType.BULLET, this.getDamage(), this.getShooter(), this.getHitstun())))
				((LivingEntity)e).setMotion(this.motion.mul(-SPEED_TRANSFER));
				this.setDead();
				return;
			}
	}
